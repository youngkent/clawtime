...
import {
  PUBLIC_DIR, DATA_DIR, MEDIA_DIR, TTS_DIR,
  BOT_NAME, RP_NAME,
  ENABLE_TASKS, TASKS_FILE,
  publicConfig,
} from './config.js';
import {
  sessions, challenges, inviteTokens,
  getSetupToken, consumeSetupToken,
  createSession,
} from './state.js';
import { checkRateLimit, setSecurityHeaders, auditLog } from './security.js';
import { loadCredentials, saveCredentials } from './credentials.js';
import {
  getRpID, getExpectedOrigin, parseBody,
  cleanExpiredSessions, cleanExpiredChallenges, MIME, ipMatches, isSafeFilename,
} from './helpers.js';
...
export async function handleRequest(req, res) {
  ...
  try {
    ...
    if (urlPath === '/api/invite' && req.method === 'POST') {
      ...
      for (const [k, v] of inviteTokens) { if (v.expiresAt < now) inviteTokens.delete(k); }
    ...
    if (urlPath === '/auth/reverify' && req.method === 'POST') {
      ...
      if (!challengeId) {
        ...
        challenges.set(cid, { challenge: options.challenge, type: 'reverify', createdAt: Date.now() });
        cleanExpiredChallenges();
    ...
    if (urlPath === '/api/tasks' && ENABLE_TASKS && TASKS_FILE) {
      ...
      const taskClientIp = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.socket.remoteAddress;
      const taskSess = sessions.get(token);
      if (!taskSess || taskSess.ip !== taskClientIp) return json(401, { error: 'Unauthorized' });
      if (req.method === 'GET') {
        try {
          const content = fs.existsSync(TASKS_FILE) ? fs.readFileSync(TASKS_FILE, 'utf-8') : '# Tasks\n\nNo tasks yet.';
    ...
    if (urlPath.startsWith('/media/') && req.method === 'GET') {
      ...
      const mimeMap = { '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.png': 'image/png', '.gif': 'image/gif', '.webp': 'image/webp', '.pdf': 'application/pdf', '.mp3': 'audio/mpeg' };
      const contentType = mimeMap[ext] || 'application/octet-stream';
      const data = fs.readFileSync(resolved);
      const headers = { 'Content-Type': contentType, 'Content-Length': data.length, 'Cache-Control': 'max-age=3600' };
    ...
    const customizableAssets = ['/icon-192.png', '/icon-512.png', '/apple-touch-icon.png', '/favicon.ico'];

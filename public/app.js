...
function showLoadMoreIndicator() {
  ...
  if (!indicator) {
    ...
    indicator.style.cssText =
      "text-align:center;padding:12px;color:#888;font-size:13px;cursor:pointer;";
...
async function init() {
  ...
  try {
    ...
    const res = await fetch("/auth/status?_=" + Date.now(), {
    ...
    authError.textContent =
      e.name === "AbortError"
        ? "Auth check timed out — try hard refresh (Cmd+Shift+R)"
  ...
  if (isRegistered) {
    ...
    if (setupToken) {
      ...
      tokenInput.style.cssText =
        "width:100%;max-width:300px;padding:12px 16px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--fg);font-size:16px;margin:12px auto;display:block;text-align:center;outline:none;";
  ...
  s = s.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, function (m, alt, url) {
    ...
    if (url.startsWith("/") || url.indexOf(location.host) !== -1) {
      ...
      if (!window._e2ePendingFetches[relUrl]) {
        ...
        setTimeout(function () {
          secureSend(JSON.stringify({ type: "fetch_resource", url: relUrl }));
        }, 0);
      }
      return (
        '<img alt="' +
        alt +
        '" style="max-width:100%;border-radius:8px;margin:6px 0;display:block" data-e2e-url="' +
        relUrl +
        '">'
      );
    }
    return (
      ...
      '" alt="' +
      alt +
      '" style="max-width:100%;border-radius:8px;margin:6px 0;display:block" loading="lazy">'
    );
  });
  // Audio player for /tts/ links: [label](/tts/file.mp3) → audio player
  s = s.replace(
    /\[([^\]]*)\]\((\/tts\/[^)]+\.mp3)\)/g,
    '<div style="margin:6px 0"><div style="font-size:13px;margin-bottom:4px">$1</div><audio controls style="width:100%;height:36px" src="$2"></audio></div>',
  );
  s = s.replace(/\[([^\]]+)\]\(([^)]+)\)/g, function (m, label, url) {
    ...
    if (isFile) {
      ...
      return (
        '<a href="' +
        url +
        '" onclick="event.preventDefault();window.openFileViewer(\'' +
        url +
        "','" +
        fname +
        '\')" rel="noopener">' +
...
function showContextMenu(x, y, bubbleEl, sender) {
  ...
  copyBtn.innerHTML =
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:3px"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg> Copy';
  copyBtn.onclick = function () {
    const rawText =
      bubbleEl.dataset.rawText || bubbleEl.innerText.replace(/\d{2}:\d{2}\s*(AM|PM)?$/m, "").trim();
...
function addMessage(text, sender, opts) {
  ...
  if (opts.replyTo) {
    ...
    qt.textContent =
      opts.replyTo.text.length > 80 ? opts.replyTo.text.slice(0, 80) + "…" : opts.replyTo.text;
    ...
    if (opts.replyTo.bubbleEl) {
      quote.addEventListener("click", function () {
        ...
        setTimeout(function () {
          opts.replyTo.bubbleEl.style.outline = "";
        }, 1500);
      });
    }
...
// ═══════════════════════════════════════════════════════════════════════════════
// API CONTRACT: Server sends DELTA text (new portion only per update).
// Client MUST accumulate deltas per messageId to build the full message.
// On 'final' state, client finalizes the message and clears tracking.
// ═══════════════════════════════════════════════════════════════════════════════
const messageAccumulator = new Map(); // messageId -> accumulated text
...
function processChatEvent(msg) {
  ...
  if (state === "delta") {
    ...
    // Handle edge case: if deltaText contains current accumulated (server sent full text), use it directly
    const current = messageAccumulator.get(messageId) || "";
    let accumulated;
    if (deltaText.startsWith(current) && deltaText.length > current.length) {
      // Server sent full accumulated text (fallback for block append) - use directly
      accumulated = deltaText;
    } else {
      // Normal delta - append
      accumulated = current + deltaText;
    }
    messageAccumulator.set(messageId, accumulated);

    if (existingBubble) {
      // Update existing bubble with accumulated text
      const existingTimeEl = existingBubble.querySelector(".msg-time");
      setBubbleContent(existingBubble, accumulated);
      if (existingTimeEl) existingBubble.appendChild(existingTimeEl);
    } else {
      // Create new bubble with data-msgid
      const bubble = addMessage(accumulated, "bot", { timestamp: Date.now() });
    ...
    // Add any final delta to accumulator
    if (deltaText) {
      const accumulated = (messageAccumulator.get(messageId) || "") + deltaText;
      messageAccumulator.set(messageId, accumulated);
    }

    const finalText = messageAccumulator.get(messageId) || "";

    if (finalText) {
      if (existingBubble) {
        // Update existing bubble with final accumulated content
        setBubbleContent(existingBubble, finalText);
        existingBubble.removeAttribute("data-msgid"); // Done streaming
      } else {
        // Create new bubble (no streaming happened)
        addMessage(finalText, "bot", { timestamp: Date.now() });
      }
    }
  ...
  switch (widgetType) {
    case "buttons": {
      ...
      buttons.forEach(function (btn) {
        if (
          btn.dataset.value === response.value ||
    ...
    case "confirm": {
      ...
      btns.forEach(function (btn) {
        if (
          (response.value && btn.classList.contains("primary")) ||
...
async function e2eInit(serverPubKeyB64) {
  try {
    ...
    const serverPubRaw = Uint8Array.from(atob(serverPubKeyB64), function (c) {
...
async function e2eDecrypt(raw) {
  try {
    ...
    const iv = Uint8Array.from(atob(msg.iv), function (c) {
      return c.charCodeAt(0);
    });
    const tag = Uint8Array.from(atob(msg.tag), function (c) {
      return c.charCodeAt(0);
    });
    const data = Uint8Array.from(atob(msg.data), function (c) {
      return c.charCodeAt(0);
    });
    ...
    const dec = await crypto.subtle.decrypt({ name: "AES-GCM", iv: iv }, e2eKey, combined);
    ...
    try {
      ...
      if (msg.type === "transcription") {
        ...
        if (window._whisperPlaceholder) {
          try {
            window._whisperPlaceholder.remove();
          } catch (_e) {
            /* ignore */
          }
      ...
      if (msg.type === "stt_error") {
        ...
        addMessage("🎤 " + (msg.error || "Voice transcription failed"), "bot", {
          timestamp: Date.now(),
        });
      ...
      if (msg.type === "reverify_request") {
        (function () {
          ...
          modal.innerHTML =
            '<div class="passphrase-card" style="max-width:360px;">' +
            "<h3>🔐 Identity Verification</h3>" +
            '<p class="desc" style="margin-bottom:12px;">' +
            reason.replace(/</g, "&lt;") +
            "</p>" +
            '<p class="desc" style="font-size:12px; margin-bottom:18px; color:var(--text-dim);">' +
            CFG.botName +
            " needs to confirm your identity before proceeding.</p>" +
          ...
          modal.onclick = function (e) {
            if (e.target === modal) document.getElementById("reverifyCancel").click();
          };
      ...
      if (msg.type === "history") {
        ...
        for (let hi = 0; hi < rawMessages.length; hi++) {
          ...
          if (historyMsg.widget) {
            historyMessages.push({
              role: "bot",
              widget: historyMsg.widget,
              timestamp: historyMsg.timestamp || null,
            });
            continue;
          }
...
function checkPageFreeze() {
  ...
  if (gap > 8000) {
    // More than 8s gap = page was frozen (interval is 5s)
    return true;
  }
  return false;
}
...
setInterval(function () {
  if (
    sessionToken &&
    chatUi.classList.contains("active") &&
...
function send() {
  ...
  if (replyTarget) {
    ...
    const quoteSnippet =
      replyTarget.text.length > 150 ? replyTarget.text.slice(0, 150) + "…" : replyTarget.text;
...
function renderAttachmentPreview() {
  ...
  imagePreview.innerHTML = pendingAttachments
    .map(function (att, i) {
      const isImage = att.type && att.type.startsWith("image/");
      const preview = isImage
        ? '<img src="' +
          att.dataUrl +
          '" style="max-height:60px;max-width:80px;border-radius:6px;object-fit:cover">'
      ...
      return (
        ...
        i +
        ')" style="position:absolute;top:-6px;right:-6px;width:18px;height:18px;border-radius:50%;border:none;background:#ef4444;color:white;font-size:12px;cursor:pointer;line-height:1">×</button>' +
        "</div>"
      );
    })
    .join("");
...
function sendWithAttachments() {
  ...
  if (hasAttachments) {
    const imageAttachments = pendingAttachments.filter(function (a) {
      return a.type && a.type.startsWith("image/");
    });
    if (imageAttachments.length > 0) {
      const imageUrls = imageAttachments.map(function (a) {
        return a.dataUrl;
      });
  ...
  if (hasAttachments) {
    ...
    secureSend(
      JSON.stringify({ type: "attachments", attachments: attachmentData, caption: sendText || "" }),
    );
  } else {
    secureSend(JSON.stringify({ type: "send", text: sendText }));
  }
...
function pttStop() {
  ...
  pttBtn.classList.remove("recording");
  pttBtn.innerHTML =
    '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="1" width="6" height="11" rx="3"/><path d="M5 10a7 7 0 0 0 14 0"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>';
  ...
  if (pttUsingWhisper) {
    ...
    if (pttMediaRecorder && pttMediaRecorder.state !== "inactive") {
      pttMediaRecorder.onstop = function () {
        ...
        if (savedBubble) {
          savedBubble.bubble.innerHTML =
            '<span class="transcribing-indicator">⏳ Transcribing with Whisper...</span>';
...
function startRecordingChunk() {
  ...
  mediaRecorder.onstop = function () {
    ...
    if (window._whisperPlaceholder) {
      ...
      window._whisperPlaceholder = addMessage("⏳ Transcribing...", "user", {
        timestamp: Date.now(),
        placeholder: true,
      });
    }
  ...
  const vadInterval = setInterval(function () {
    ...
    if (rms > 0.07) {
      // Balanced: responsive but filters noise
      ...
      if (!speechStarted) {
        ...
        if (!window._whisperPlaceholder) {
          window._whisperPlaceholder = addMessage("🎤 Recording...", "user", {
            timestamp: Date.now(),
            placeholder: true,
          });
        }
...
function endCall() {
  ...
  if (window._micStream) {
    window._micStream.getTracks().forEach(function (t) {
      t.stop();
    });
...
const CALL_NOISE_WORDS =
  /^(um+|uh+|hm+|hmm+|ah+|oh+|er+|huh+|mhm+|mm+|shh+|tsk|psst|ha(ha)*|he(he)*|ho(ho)*)$/i;
...
function stopBargeInVAD() {
  ...
  if (bargeInStream) {
    bargeInStream.getTracks().forEach(function (t) {
      t.stop();
    });
    bargeInStream = null;
  }
...
function playNextTTSChunk() {
  ...
  if (!callUseWhisper && SpeechRecognitionAPI && callRecognition) {
    try {
      ...
      /* ignore - recognition may not be available */
    }
  }
...
if (avatarPanelEl) {
  ...
  avatarPanelEl.addEventListener("click", function (e) {
    ...
    if (
      e.target.closest(".avatar-buttons") ||
      e.target.closest(".avatar-action-btn") ||
      e.target.closest("#callEndBtn") ||
      e.target.closest("#callOverlay") ||
...
(function () {
  ...
  if (taskBtnHeader)
    taskBtnHeader.addEventListener("click", function (e) {
      e.stopPropagation();
      openTasks();
    });
...
(function () {
    ...
    fetch("/auth/passkeys", {
      ...
      .then(function (data) {
        ...
        data.passkeys.forEach(function (pk) {
          const div = document.createElement("div");
          div.style.cssText =
            "display:flex; align-items:center; justify-content:space-between; padding:10px 12px; background:var(--surface2); border:1px solid var(--border); border-radius:8px; margin-bottom:8px;";
          const info = document.createElement("div");
  ...
  if (addPasskeyBtn) {
    addPasskeyBtn.addEventListener("click", async function () {
      ...
      try {
        ...
        const cred = await SimpleWebAuthnBrowser.startRegistration({
  ...
  let currentAvatar =
    (window.CLAWTIME_THEME && window.CLAWTIME_THEME.id) || localStorage.getItem("selectedAvatar");
  let previewingAvatar = null;
  let previewAnimationId = null;

  function getAvatarData(id) {
    const found =
      availableAvatars.find(function (a) {
        return a.id === id;
      }) || availableAvatars[0];
  ...
  function renderAvatarList() {
    ...
    availableAvatars.forEach(function (av) {
      ...
      card.className =
        "avatar-card" + (isSelected ? " selected" : "") + (isPreviewing ? " previewing" : "");

      let html =
        '<div class="avatar-card-emoji">' +
        av.emoji +
        "</div>" +

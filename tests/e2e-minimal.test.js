...
describe('Minimal E2E', () => {
  ...
  test('should complete full message round-trip', async () => {
    ...
    await new Promise((resolve, reject) => {
      ...
      client.on('message', (raw) => {
        ...
        if (msg.type === 'auth_ok') {
          ...
          client.send(JSON.stringify({ type: 'e2e_key', clientPublicKey: clientECDH.getPublicKey('base64') }));
        }
    ...
    const chatResponse = await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        console.log('[Client] Messages received so far:', messages.map(m => m.type));
  ...
  test('should pull missed messages on reconnect via history', async () => {
      ...
      client1.on('message', (raw) => {
        ...
        try {
          ...
          if (parsed._e2e && e2eKey1) {
            const decipher = crypto.createDecipheriv('aes-256-gcm', e2eKey1, Buffer.from(parsed.iv, 'base64'));
            decipher.setAuthTag(Buffer.from(parsed.tag, 'base64'));
            msgStr = Buffer.concat([decipher.update(Buffer.from(parsed.data, 'base64')), decipher.final()]).toString('utf8');
          }
        } catch {}
        const msg = JSON.parse(msgStr);
        if (msg.type === 'auth_ok') {
          ...
          e2eKey1 = Buffer.from(crypto.hkdfSync('sha256', shared, 'clawtime-e2e-salt', 'clawtime-e2e-key', 32));
          client1.send(JSON.stringify({ type: 'e2e_key', clientPublicKey: ecdh.getPublicKey('base64') }));
        }
        if (msg.type === 'e2e_ready') { clearTimeout(timeout); resolve(); }
      });
    ...
    function encrypt1(text) {
      ...
      return JSON.stringify({ _e2e: true, iv: iv.toString('base64'), tag: cipher.getAuthTag().toString('base64'), data: enc.toString('base64') });
    }
      ...
      client2.on('message', (raw) => {
        ...
        try {
          ...
          if (parsed._e2e && e2eKey2) {
            const decipher = crypto.createDecipheriv('aes-256-gcm', e2eKey2, Buffer.from(parsed.iv, 'base64'));
            decipher.setAuthTag(Buffer.from(parsed.tag, 'base64'));
            msgStr = Buffer.concat([decipher.update(Buffer.from(parsed.data, 'base64')), decipher.final()]).toString('utf8');
          }
        } catch {}
        ...
        if (msg.type === 'auth_ok') {
          ...
          ecdh.generateKeys();
          const shared = ecdh.computeSecret(Buffer.from(msg.serverPublicKey, 'base64'));
          e2eKey2 = Buffer.from(crypto.hkdfSync('sha256', shared, 'clawtime-e2e-salt', 'clawtime-e2e-key', 32));
          client2.send(JSON.stringify({ type: 'e2e_key', clientPublicKey: ecdh.getPublicKey('base64') }));
        }
        if (msg.type === 'e2e_ready') { clearTimeout(timeout); resolve(); }
      });

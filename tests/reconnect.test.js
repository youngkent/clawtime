            ...
            setTimeout(() => {
              if (ws.readyState === WebSocket.OPEN) {
                ws.send(JSON.stringify({
                  ...
                  payload: { sessionKey: 'agent:main:main', runId, state: 'final', message: { content: [{ type: 'text', text: 'OK' }] }}
                }));
              }
            }, 50);
          }
  ...
  test('E2E handshake completes successfully', async () => {
    ...
    const authOk = await new Promise((res, rej) => {
      ...
      ws.on('message', function h(raw) {
        ...
        if (m.type === 'auth_ok') { clearTimeout(t); ws.off('message', h); res(m); }
      });
    });
    ...
    const e2eKey = Buffer.from(crypto.hkdfSync('sha256', secret, 'clawtime-e2e-salt', 'clawtime-e2e-key', 32));
    ...
    const decrypt = (raw) => {
      ...
      return JSON.parse(Buffer.concat([d.update(Buffer.from(p.data, 'base64')), d.final()]).toString());
    };
    ...
    const readyPromise = new Promise((res, rej) => {
      ...
      ws.on('message', function h(raw) {
        try {
          const m = decrypt(raw);
          if (m.type === 'e2e_ready') { clearTimeout(t); ws.off('message', h); res(m); }
        } catch {}
      });
    });
  ...
  test('encrypted message can be sent after E2E', async () => {
    ...
    const readyPromise = new Promise((res, rej) => {
      ...
      ws.on('message', function h(raw) {
        try {
          const m = decrypt(raw);
          if (m.type === 'e2e_ready' || m.type === 'connected') { clearTimeout(t); ws.off('message', h); res(m); }
        } catch {}
      });
    });
    ...
    const response = await new Promise((res, rej) => {
      ...
      ws.on('message', function h(raw) {
        try {
          ...
          if (dec.type === 'avatar_state') { clearTimeout(t); ws.off('message', h); res(dec); }
        } catch {}
      });
    });
